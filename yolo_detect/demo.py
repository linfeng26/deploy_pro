...
import argparse
import os
from util import parse_xml, parse
...
parser.add_argument('--model_dir',type=str, default = '/home/dang.hua/code/bkd/window/deploy/yolo_detect/models/LDRN_NYU_ResNext101_pretrained_data.pkl')
parser.add_argument('--img_dir', type=str, default = 'test.jpg')
...
def depth_estimation(detect_result, img_dir):
    # img_dir = detect_result['img_path']
    ...
    # %%
    args.img_dir = img_dir  # 测试图片路径

    if args.img_dir is not None:
        if args.img_dir[-1] == '/':
            args.img_dir = args.img_dir[:-1]
        img_list = [args.img_dir]
        result_filelist = ['/disk1/ai/bkd/window/deploy/yolo_detect/media/output/out_' + args.img_dir.split('/')[-1]]
    ...
    for i, img_file in enumerate(img_list):
        ...
        # distace = parse_xml(args.img_dir, out)
        distace = parse(detect_result, out)
        
        # if distace == float("inf"):
        #     print('图片中不存在窗户或人')
        # for key in distace:
        #     print(key, '相对距离:', round(distace[key], 3), '距离较近' if distace[key] < 0.1 else '距离较远')
...
if __name__ == '__main__':
    ...
    "res": [
        {
        "confidence": 0.7731348872184753,
        "x": 0.24531249701976776,
        "y": 0.3777777850627899,
        "width": 0.06458333134651184,
        "height": 0.5138888955116272,
        "className": "people"
        },
        {
        "confidence": 0.9248427152633667,
        "x": 0.5729166716337204,
        "y": 0.3212963044643402,
        "width": 0.12708333134651184,
        "height": 0.6787037253379822,
        "className": "people"
        }
    ],
    "img_path": "media/upload/test.jpg"
    }
    depth_estimation(detect_result)
