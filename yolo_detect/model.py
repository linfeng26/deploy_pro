...
class Lap_decoder_lv5(nn.Module):
    ...
    def forward(self, x, rgb):
        ...
        lap_lv5 = torch.sigmoid(self.decoder1(dense_feat))  # 限制范围, 更好训练
        lap_lv5_up = self.upscale(lap_lv5, scale_factor = 2, mode='bilinear')   # 上采样

        # decoder 2 - Pyramid level 4
        dec2 = self.decoder2_up1(dense_feat)
        dec2 = self.decoder2_reduc1(torch.cat([dec2,cat3],dim=1))
        # 接下来4层卷积, 最后一层再加上lv4
        dec2_up = self.decoder2_1(torch.cat([dec2,lap_lv5_up,rgb_lv4],dim=1))   # 252 + 1 + 3 = 256 个特征图
...
class LDRN(nn.Module):
    ...
    def forward(self, x):
        out_featList = self.encoder(x)  # encoder 的 forward 函数返回多层特征图
        # interpolate 的 scale_factor 参数指定倍率 < 1 下采样, > 1 上采样
